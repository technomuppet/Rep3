    ...
    fun loadRegions(context: Context, isFront: Boolean): Map<MuscleRegion, Path> {
        ...
        val inputStream: InputStream = try {
            ...
            AnatomyDiagnostics.auditSvgLoad(
                ...
                expectedIds = 43,
                loadedIds = 0,
                missingIds = listOf(assetName),
                exceptionMsg = e.message ?: "Asset open exception"
            )
            // If asset is missing, return empty map but the failure is recorded (not suppressed silently)
            return emptyMap()
        }
        ...
        try {
            ...
            // The exact exception is recorded via audit, not suppressed silently.
            AnatomyDiagnostics.auditSvgLoad(
                assetName = assetName,
                expectedIds = 43,
                loadedIds = paths.size,
                missingIds = emptyList(),
                exceptionMsg = e.message ?: "SVG parsing exception"
            )
        } finally {
